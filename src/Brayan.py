...
class MyVisitor(ParseTreeVisitor):
    ...
    def visitAsigDatos(self, ctx: UNaIAParser.AsigDatosContext):
        ...
        if len(res) > 1:
            if res[1] != "None":
                ...
                instruction += "caracteristicas_"+str(ctx.ID())+" = "+str(ctx.ID())+'.drop("' + res[1] + '", axis=1)\n'
            else:
    ...
    def visitAsigModelo(self, ctx: UNaIAParser.AsigModeloContext):
        ...
        if res == 'regresionLogistica':
            ...
            instruction += str(ctx.ID()) + " = autosklearn.classification.AutoSklearnClassifier(time_left_for_this_task=30)"
    ...
    def visitDivision(self, ctx: UNaIAParser.DivisionContext):
        ...
        if porcentaje > 100 or porcentaje < 0:
            ...
            instruction += "caracteristicas_" +str(ctx.parentCtx.ID(0)) + ", etiquetas_" +str(ctx.parentCtx.ID(0)) + ", caracteristicas_" +str(ctx.parentCtx.ID(1)) + ", etiquetas_" +str(ctx.parentCtx.ID(1))
            instruction += " = train_test_split(caracteristicas_"+str(ctx.ID())+", etiquetas_"+str(ctx.ID())+", test_size="+str(porcentaje/100)+")\n"
    ...
    def visitEntrenamiento(self, ctx: UNaIAParser.EntrenamientoContext):
        ...
        instruction += str(ctx.ID(0)) + ".fit(" + "caracteristicas_" +str(ctx.ID(1))+", etiquetas_"+str(ctx.ID(1))+  ")\n\n"
    ...
    def visitEvaluacion(self, ctx: UNaIAParser.EvaluacionContext):
        ...
        instruction+= "desempeño_"+str(ctx.ID(0))+"= "+str(ctx.ID(0))+".score("+"caracteristicas_" +str(ctx.ID(1))+", etiquetas_"+str(ctx.ID(1))+")\n"
        print("#####################################################")
    ...
    def visitReporte(self, ctx: UNaIAParser.ReporteContext):
        ...
        instruction += "pd.option_context('display.max_rows', "+str(rows)+", 'display.max_columns', "+str(cols)+"):\n"
